...
TEX_SCHEMA_FILE = os.path.join(
    os.path.dirname(__file__), "../paper/includes/api_schema.tex"
)
...
def get_request_body_type(request_body: dict) -> str:
    ...
    if (
        "requestBody" not in request_body
        or "content" not in request_body["requestBody"]
    ):
        return ""
...
def get_response_type(responses: dict) -> str:
    ...
    success_codes = [
        code for code in responses["responses"].keys() if code.startswith("2")
    ]
...
\\textbullet\\ \\texttt{{{escaped_path}}}:"""
    for method, method_attrs in methods.items():
        ...
        escaped_summary = (
            escape_latex(method_attrs["summary"]) if "summary" in method_attrs else ""
        )
        escaped_description = (
            escape_latex(method_attrs["description"])
            if "description" in method_attrs
            else ""
        )
        escaped_request_body = (
            escape_latex(request_body_type) if request_body_type else ""
        )
        ...
        if escaped_request_body:
            lines.append(
                f"\\textbf{{Request Body:}} \\texttt{{{escaped_request_body}}}"
            )
        if escaped_response:
