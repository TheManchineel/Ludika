...
if os.getenv("TAVILY_API_KEY") is None:
    if get_config_value("GenerativeAI", "tavily_api_key") is None:
        raise ValueError(
            "TAVILY_API_KEY is not set and tavily_api_key is not set in config.ini"
        )
...
def wikipedia_search(query: str) -> str:
    ...
    try:
        ...
        if result and result.strip():
            ...
            return f"No Wikipedia articles found for '{query}'. Try using a different search term or check the spelling."
...
def fetch_page_content(url: str) -> str:
    ...
    try:
        ...
        if is_private_ip(url):
            return "Error: Attempting to access a private IP address."
...
def create_game_with_fixed_url(
    fixed_url: str, game_added_callback: Callable | None = None
):
    ...
    def create_game_fixed(
        name: str, description: str, tags: Optional[list[int]] = None
    ) -> dict:
        print(f"Creating game: {name}, {description}, {fixed_url}, tags: {tags}")
        try:
            with db_context() as session:
                if tags:
                    tag_objects = session.exec(
                        select(Tag).where(Tag.id.in_(tags))
                    ).all()
                else:
                    tag_objects = []

                game_data = GameCreate(
                    name=name, description=description, url=fixed_url, tags=tags or []
                )
                ...
                try:
                    ...
                    if new_image:
                        ...
                        get_logger().info(
                            f"Successfully added image {new_img_id} to game {db_game.id}"
                        )
                except Exception as e:
                    get_logger().warning(
                        f"Failed to add image to game {db_game.id}: {str(e)}"
                    )
...
def generate_game_object_with_fixed_url(fixed_url: str):
    ...
    def generate_game_object_fixed(
        name: str, description: str, tags: Optional[list[int]] = None
    ) -> dict:
        """
        ...
        """
        print(
            f"Generating GameCreate object: {name}, {description}, {fixed_url}, tags: {tags}"
        )
        game_object = GameCreate(
            name=name, description=description, url=fixed_url, tags=tags or []
        )
        return {
            "success": True,
