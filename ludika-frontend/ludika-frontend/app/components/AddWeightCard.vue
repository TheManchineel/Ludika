<template>
    <div class="add-weight-card">
        <div v-if="!showDropdown" class="add-weight-placeholder" @click.stop="openDropdown">
            <div class="placeholder-content">
                <font-awesome-icon icon="plus" class="plus-icon" />
                <span class="placeholder-text">Add Criterion Weight</span>
            </div>
        </div>
...
const handleClickOutside = (event: Event) => {
    ...
    if (!target.closest('.add-weight-card')) {
        showDropdown.value = false
    }
}
...
.add-weight-placeholder:hover {
    border-color: var(--va-primary);
    background: rgba(148, 35, 224, 0.05);
}
